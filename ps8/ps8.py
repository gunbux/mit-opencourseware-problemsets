        ...
        if random.random() > probability:
            ...
            for drug in self.resistances:
                if random.random() < self.mutProb:
                    ...
                    elif self.resistances.get(drug) == False:
...
def simulationWithDrug(timing):
    ...
    for i in range(100):
        ...
        #print(f"Virus resistance to guttagonol is {virus.isResistantTo('guttagonol')}")
        virusList.append(virus)

    patient = Patient(virusList,1000)
    #print('Running simulation')
    for time in range(timing):
        #print(f'Current time is {time}, resistant pop is {patient.getResistPop(patient.getPrescriptions())}')
        x_values.append(time)
        y_values.append(patient.update())
        #z_values.append(0)

    #print('Drug introduced')
    patient.addPrescription('guttagonol')

    for time in range(timing,300):
        #print(f'Current time is {time}, resistant pop is {patient.getResistPop(patient.getPrescriptions())}')
        #z_values.append(patient.getResistPop(patient.getPrescriptions()))
        x_values.append(time)
        y_values.append(patient.update())
...
#
#    plt.plot(x_values,z_values,label = 'Current Resistant Population')
#    plt.ylabel('Resistant Population')
#    plt.xlabel('Time')
#    plt.legend()
    #plt.show()
...
def simulationDelayedTreatment():
    ...
    condition_0=[]
    for i in range(30):
        condition_300.append(simulationWithDrug(300))
        condition_150.append(simulationWithDrug(150))
        condition_75.append(simulationWithDrug(75))
        condition_0.append(simulationWithDrug(0))
    print(f'Current stats are {condition_300},{condition_150},{condition_75},{condition_0} ')
